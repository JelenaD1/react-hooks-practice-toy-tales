...
function App() {
  ...
  const handleSubmit = (toyForm) => {
    ...
    fetch("http://localhost:3001/toys", {
      ...
      body: JSON.stringify(toyForm)//or configObject
    })
      .then(resp => resp.json())
  ...
  return (
    <>
      <Header />
      {showForm ? <ToyForm onClickSubmit={handleSubmit} setToyList={setToyList} toyList={toyList} /> : null}
      <div className="buttonContainer">
        <button onClick={handleClick}>Add a Toy</button>
      </div>
      <ToyContainer onClickLike={handleLikes} onClickDelete={deleteToy} toyList={toyList} setToyList={setToyList} />
    </>
  );
}
